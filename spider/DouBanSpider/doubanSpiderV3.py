...
def book_spider(book_tag):
    ...
    while(1):
        # url='http://www.douban.com/tag/%E5%B0%8F%E8%AF%B4/book?start=0' # For
        # Test
        url = 'http://www.douban.com/tag/' + \
            urllib.parse.quote(book_tag) + '/book?start=' + str(page_num * 15)
...
def get_people_num(url):
    ...
    try:
        req = urllib.request.Request(
            url, headers=hds[np.random.randint(0, len(hds))])
        source_code = urllib.request.urlopen(req).read()
        plain_text = str(source_code)
